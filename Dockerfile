# Common-test-MCP container — built primarily so Glama (and any other MCP
...
# JSON-RPC response.
#
# Day-to-day use stays `uvx Common-test-MCP` on the host: real test runs
...
ENTRYPOINT ["python", "-m", "common_test_mcp.server"]
