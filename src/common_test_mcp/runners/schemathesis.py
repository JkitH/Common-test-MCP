...
Schemathesis is an optional dep: `pip install 'Common-test-MCP[api]'`.
...
class SchemathesisRunner(TestRunner):
    ...
    def _base_cmd(self, cli: str, url: str, report_json: Path) -> list[str]:
        ...
        # and translate to Common-test-MCP's report.json shape from there.
    ...
    def _normalize_report(self, _legacy_arg: Path | None = None) -> None:
        ...
        # them but Common-test-MCP's existing pipeline only knows passed/failed/skipped.
        _ = errors
        report = {
            "summary": {
                "total": len(tests),
