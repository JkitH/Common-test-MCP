...
- Common-test-MCP 的 optimizer 看到連續因 CAPTCHA 失敗 -> 分類為 `external` 而非 `broken` / `flaky`
- CI 階段 skip CAPTCHA 後路徑，local dev 走 Tier 3 手動或 AI 視覺判斷

### Tier 3：AI 視覺判斷 (v0.7.0+) — 最後手段

Common-test-MCP 計劃中的 `solve_visual_challenge` tool：
...
- Common-test-MCP's optimizer classifies consecutive CAPTCHA-caused failures as `external` rather than `broken` or `flaky`
...
The planned `solve_visual_challenge` tool in Common-test-MCP:
...
_BUILTIN_HEADER_ZH = (
    ...
    "> 這份是 Common-test-MCP 內建的通用 QA 方法論，整理自 ISTQB / Google 測試金字塔 / 業界 mobile QA 標準\n"
...
_BUILTIN_HEADER_EN = (
    "# QA Knowledge — Universal Testing Methodology (built-in fallback)\n\n"
    "> This is Common-test-MCP's built-in universal QA methodology, distilled from\n"
...
_STARTER_HEADER_ZH = (
    ...
    "> 給 Common-test-MCP 讀的領域知識。get_qa_context() 會把這份內容暴露給 AI，\n"
    ...
    "> 由 Common-test-MCP 預載。一般不需要動；場景不適用某些方法論可以刪除對應 H2 段落。\n"
    "> \n"
...
_STARTER_HEADER_EN = (
    "# QA Knowledge — {project_name}\n\n"
    "> Domain knowledge for Common-test-MCP to read. `get_qa_context()` exposes this\n"
    ...
    "> preloaded by Common-test-MCP. You usually don't need to touch it; delete any\n"
