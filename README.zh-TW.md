...
- **原生 API 測試 — 兩個 runner**（v0.6.0 / v0.6.1 起）：API 測試這格目前並列兩個 runner，各自吃你團隊已經在維護的 artifact。
  ...
  - **Newman**（`QA_RUNNER=newman`，v0.6.1 起）：餵 runner 一個 Postman 2.x 匯出的 collection（可選帶 environment / globals），Newman 會 replay 每個 request、執行內嵌的 `pm.test(...)` assertion，每個 assertion 對應一個 Common-test-MCP nodeid。**Newman 是系統依賴**（`npm install -g newman`）—— 它是 npm 套件不是 pip，所以沒有 Python extra 可裝。
...
在 client config 加 `Common-test-MCP`，不用全域裝任何東西；[`uv`](https://docs.astral.sh/uv/) 會每次幫你抓並在隔離環境跑：
...
整個設定就這樣。第一次呼叫會下載，後續走 cache。要指定版本：`uvx Common-test-MCP@0.4.1 ...`。

### B. 裝進自己的 venv（給貢獻者 / 想 hack 的人）

```bash
pip install Common-test-MCP       # 或 git clone 後 pip install -e .
...
| `schemathesis` / `api` | `pip install 'Common-test-MCP[api]'`（自動拉 `schemathesis>=3.0,<4`） |
...
Common-test-MCP nodeid。結果跟 Schemathesis、UI 測試共用同一條 `report.json` /
...
沒有 `pip install 'Common-test-MCP[postman]'` extra——runner 只是 shell out
...
> *當後端 bypass 不可行時：Claude 看 CAPTCHA、Common-test-MCP 負責點下去。*
...
```
Common-test-MCP.inspect_visual_challenge()   # 截圖 + tile 網格
→ AI 視覺判斷後選 [0, 4, 7]
...
1. 在 `src/common_test_mcp/runners/` 新增 `your_runner.py`，繼承 `TestRunner`，實作必要的 abstract method
...
[**直接看實際渲染 →**](https://htmlpreview.github.io/?https://github.com/JkitH/Common-test-MCP/blob/main/sample_report.html)
...
`Common-test-MCP` **不打包**任何第三方 SDK——保持「測試執行 + 分析」單一職責。實務上 QA 工作流是**多個 MCP server 並存**、由 Claude 自動編排跨 server 的 tool chain 達成的。MCP 協議本身沒有 server-to-server RPC，每個 server 互不知曉彼此存在，AI client 才是指揮。
...
為什麼這樣設計：`Common-test-MCP` 專注做「測試這個迴圈」（analyze → generate → run → coach）。JIRA / Slack / Sentry 各自有專業 server 維護，硬塞進這個 repo 只會稀釋焦點、重複處理 auth、強迫所有使用者繼承不需要的依賴。

---

## 支持這個專案 ☕

`Common-test-MCP` 是我一人在下班和週末維護的。如果它幫你省了時間，或改變了你們團隊看 AI-driven QA 的方式，一杯咖啡能讓凌晨 debug Maestro 的夜晚撐下去：
...
MIT © 2026 JkitH — 英文原版（具法律效力）見 [`LICENSE`](LICENSE)；
