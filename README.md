...
<h1 align="center">Common-test-MCP</h1>

<p align="center">
  <em>Common-test-MCP — your AI QA loop, from analyze to advise.</em>
</p>
...
<p align="center">
  ...
  <a href="https://github.com/JkitH/Common-test-MCP/actions/workflows/ci.yml"><img src="https://github.com/JkitH/Common-test-MCP/actions/workflows/ci.yml/badge.svg" alt="CI" /></a>
  <a href="https://glama.ai/mcp/servers/JkitH/Common-test-MCP"><img src="https://glama.ai/mcp/servers/JkitH/Common-test-MCP/badges/score.svg" alt="Glama score" /></a>
...
- **Native API testing — two runners** (since v0.6.0 / v0.6.1): two peers
  ...
  - **Newman** (`QA_RUNNER=newman`, since v0.6.1): point at an exported
    ...
    assertions, and returns one Common-test-MCP nodeid per assertion. Newman
...
Add `Common-test-MCP` to your client config without installing anything globally; [`uv`](https://docs.astral.sh/uv/) fetches and runs it in an ephemeral environment per session:
...
That's the whole setup. First call downloads the package; subsequent calls are cached. Switching versions: `uvx Common-test-MCP@0.4.1 ...`.

### B. Install into a project venv (for contributors / hacking)

```bash
pip install Common-test-MCP       # or: pip install -e . from a clone
...
| `schemathesis` / `api` | `pip install 'Common-test-MCP[api]'` (pulls in `schemathesis>=3.0,<4`) |
...
returns one Common-test-MCP "test" per assertion. Results flow through the
...
There's no `pip install 'Common-test-MCP[postman]'` extra — the runner
...
> *When backend bypass isn't an option: Claude looks at the CAPTCHA, Common-test-MCP does the clicks.*
...
```
Common-test-MCP.inspect_visual_challenge()  # screenshot + tile grid
...
1. Create `src/common_test_mcp/runners/your_runner.py`, subclass `TestRunner`,
...
[**Open the live rendered demo →**](https://htmlpreview.github.io/?https://github.com/JkitH/Common-test-MCP/blob/main/sample_report.html)
...
`Common-test-MCP` doesn't bundle third-party SDKs — it stays a pure
...
All five run as separate processes alongside `Common-test-MCP`:
...
Why this matters: `Common-test-MCP` stays focused on the test loop
...
本 repo 不打包任何第三方 SDK——維持「測試執行 + 分析」單一職責。實務上 QA 工作流是**多個 MCP server 並存、由 Claude 編排跨 server 的 tool chain**達成的。範例配套：JIRA / Slack / GitHub / Sentry / Filesystem 各自獨立 MCP server，配上 `Common-test-MCP` 拼出完整測試管線。
...
`Common-test-MCP` is built and maintained solo on nights and weekends. If it saved you time or shaped how your team thinks about AI-driven QA, a coffee keeps the late-night Maestro debugging sessions going:
...
This repo is **maintained solo**. Ideas and bug reports are very welcome — please open an [Issue](https://github.com/JkitH/Common-test-MCP/issues/new/choose) or start a [Discussion](https://github.com/JkitH/Common-test-MCP/discussions). I read every one and will implement what fits the project's direction.
...
MIT © 2026 JkitH — see [`LICENSE`](LICENSE)
