...
Common-test-MCP ships two native API runners as of v0.6.1:
...
API with Common-test-MCP v0.6.0 using the bundled 3-endpoint sample. By the
...
```bash
pip install 'Common-test-MCP[api]'
```
...
> **You**: Which runner is Common-test-MCP using right now?
...
There's no `Common-test-MCP[postman]` extra to install. The runner shells
...
The JSON report Newman emits gets translated into Common-test-MCP's
`report.json` shape: **one Common-test-MCP "test" per pm.test assertion**.
