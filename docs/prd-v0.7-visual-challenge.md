# Common-test-MCP v0.7 — AI Visual Challenge Solver

**Status:** Draft v0.1 · **Author:** JkitH (JkitH) · **Last updated:** 2026-05-18 · **Target ship:** v0.7.0 within 2 weeks
...
Chinese brand stays **Common-test-MCP**. No rebrand.
...
- **The MCP family is now demonstrably multimodal-ready**. Anthropic shipped Claude Sonnet 4 vision; GPT-4o is standard at OpenAI; Gemini 2.5 ships native vision. Every major AI client Common-test-MCP integrates with has vision. The architectural assumption is no longer speculative.
...
**Tertiary:** Manual testers using Common-test-MCP as a power-user remote-control during exploratory sessions.
...
A single composite tool would have to block waiting for the AI client's vision response, then click — but MCP tool calls are atomic and serialized. The two-tool design lets each call return immediately; the AI client decides between them. This matches every other tool in Common-test-MCP (`run_tests`, `get_optimization_plan`, etc.) — each is a discrete decision boundary.
...
**No MCP-to-MCP RPC.** Same as the rest of Common-test-MCP.
...
The CAPTCHA solver participates in Common-test-MCP's existing optimizer pipeline:
...
| R4 | Solving CAPTCHAs becomes the "thing people use Common-test-MCP for" — distracts from core QA narrative | Keep the feature opt-in by env var. Position as Tier 3 escape hatch, not headline capability. README leads with web/mobile/API testing; CAPTCHA solver is below the fold. |
...
- Show HN at v0.7.0 ship: `Show HN: Common-test-MCP v0.7 — Claude solves reCAPTCHA through this MCP, no new vendor`
...
| Tagline in README | "When backend bypass isn't an option: Claude looks at the CAPTCHA, Common-test-MCP does the clicks." |
...
A solo dev running Common-test-MCP against their client's staging site, with written authorization, where backend bypass isn't available.
...
```
Common-test-MCP.inspect_visual_challenge(page_id="active")
  → {
      challenge_id: "ed1f7-...",
      screenshot_base64: "...",
      challenge_text: "Select all images with traffic lights",
...
*End of PRD v0.1 for Common-test-MCP v0.7. Discussion in Common-test-MCP Issues once a draft is opened.*
