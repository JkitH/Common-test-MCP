# Common-test-MCP v0.6 — Native API Testing

**Status:** Draft v0.1 · **Author:** JkitH (JkitH) · **Last updated:** 2026-05-17 · **Target ship:** v0.6.0 (Phase 1) within 2 weeks
...
Today Common-test-MCP drives web (pytest / Jest / Cypress / Go) and mobile (Maestro) test suites natively. API testing is **implicit-only** — your existing pytest-with-`httpx` or Jest-with-`supertest` tests run, but there's no dedicated API runner, no OpenAPI introspection, no contract-test surface. That gap is what the family-site copy is currently hedging around.

v0.6 ships **native API testing** as the third capability of the family's execution layer. Three runners across two phases:

1. **`schemathesis`** (v0.6.0) — OpenAPI-driven test generation. Hand it a schema URL, get auto-generated API tests, run them, return structured results into Common-test-MCP's standard history / flake / coach pipeline.
...
Chinese brand: continues as **Common-test-MCP**. No rebrand.
...
| Position | Truth |
|---|---|
| **"Common-test-MCP tests web + mobile + API"** (current copy) | API is **inferred** — there's no dedicated API runner |
| **"Just write pytest with `httpx`"** (current advice) | True but underwhelming — every Python QA team already does this; nothing about Common-test-MCP uniquely enables it |
...
**In scope (v0.6.0):**
- New runner: `src/common_test_mcp/runners/schemathesis.py`
...
| `run_tests` | Invokes `schemathesis run --checks <X> <openapi_url>`. Parses JSON output into Common-test-MCP report shape |
...
class SchemathesisRunner:
    ...
    def run_tests(self, filter: str | None = None, **kwargs) -> dict:
        ...
        Parse JSON output → Common-test-MCP standard report shape.
...
Schemathesis emits its own JSON report shape. We map to Common-test-MCP's `report.json` (already used by pytest / Jest / etc. via `pytest-json-report`).

Mapping table:

| Schemathesis field | Common-test-MCP field |
|---|---|
...
2.  Common-test-MCP.get_runner_info()                  → current: schemathesis
3.  Common-test-MCP.run_tests()                        → 24 endpoints × 5 checks = 120 cases
                                                       8 failed (3 endpoints have schema violations)
4.  Common-test-MCP.get_optimization_plan()            → broken: POST /pet (3 consec fails, same sig)
                                                       flaky: GET /pet/{id} (PFPFP pattern)
5.  Common-test-MCP.get_failure_details("POST /pet :: response_conformance")
                                                    → request body, response body, violation
6.  user fixes the API in their IDE
7.  Common-test-MCP.run_failed()                       → now 0 failures
...
Common-test-MCP (QA_RUNNER=schemathesis) → run_tests → coverage
```
...
| **Compatibility** | Python 3.10+, schemathesis>=3.0, MCP SDK >=1.0.0. Optional dep — installs only when user does `pip install 'Common-test-MCP[api]'`. |
...
| R4 | The "we test web + mobile + API" copy bumps Common-test-MCP into more competitive comparison (vs. specialist API testing vendors) | Stay honest about scope — we're a runner orchestrator, not a Postman replacement |
...
- Common-test-MCP starts being listed on "MCP for API testing" comparison posts
...
- License: MIT (mirror existing Common-test-MCP)
...
- Optional dep group (`pip install 'Common-test-MCP[api]'`) so the base install stays small
...
- Show HN on v0.6.0 ship day: "Show HN: Common-test-MCP v0.6 — Schemathesis runner so Claude can drive OpenAPI-driven API tests"
...
Common-test-MCP.get_optimization_plan()
...
Common-test-MCP.get_failure_details("POST /users :: response_schema_conformance")
  → request:
      POST /users
...
3. **Optional dep vs hard dep** — should `schemathesis` install with `pip install Common-test-MCP` (hard) or only `pip install 'Common-test-MCP[api]'` (optional)? Recommendation: optional, keeps base install slim.
...
3. **Dependency model** — `schemathesis` is an **optional dep**. Users install via `pip install 'Common-test-MCP[api]'`. Base install stays slim; the runner module imports schemathesis lazily and raises a clear `ImportError` with the install hint if missing.
...
4. **Report parsing** — use Newman's JSON reporter via
   ...
   Common-test-MCP nodeid per `pm.test(...)` call.
...
*End of PRD v0.1 for Common-test-MCP v0.6. Discussion in Common-test-MCP Issues once a draft is opened.*
