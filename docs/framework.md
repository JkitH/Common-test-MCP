...
[project.scripts]
Common-test-MCP = "common_test_mcp.server:run"
...
### `src/common_test_mcp/tools/runner.py`
...
### `src/common_test_mcp/tools/reporter.py`
...
### `src/common_test_mcp/tools/generator.py`
...
```bash
# 建立專案
mkdir Common-test-MCP && cd Common-test-MCP
...
```json
{
  "mcpServers": {
    "Common-test-MCP": {
      ...
      "cwd": "/absolute/path/to/Common-test-MCP",
      "env": {
        "QA_PROJECT_ROOT": "/absolute/path/to/Common-test-MCP/tests_project"
      }
    }
  }
}
...
重啟 Claude Desktop，左下角應該看到 `Common-test-MCP` 的工具圖示。
